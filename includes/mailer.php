...
function emailGuarantorRequest(string $guarantorName, string $borrowerName, string $loanType, float $amount, int $months): string {
    $amountFmt = '&#8358;' . number_format($amount, 2);
    $body = "
    <p>Dear <strong>$guarantorName</strong>,</p>
    <p>Your fellow MDCAN Cooperative member, <strong>$borrowerName</strong>, has selected you as a <strong>Guarantor</strong> for a loan application.</p>
    <div class='info-box'>
    ...
    <div style='background:#fff3cd;border-left:4px solid #ffc107;padding:15px 20px;border-radius:4px;margin:20px 0'>
        <p style='margin:0 0 8px;color:#856404;font-weight:bold'>Guarantor Declaration</p>
        <p style='margin:0;color:#856404;font-style:italic'>\"I, hereby declare that, in case of any default by the borrower, I agree to be wholly or partially responsible for the payment.\"</p>
    </div>
    <p>You are required to <strong>accept</strong> or <strong>reject</strong> this guarantor request. Please log in to your Member Dashboard — a consent form is waiting for your response.</p>
    <a href='" . BASE_URL . "/member/dashboard.php' class='btn'>Respond on My Dashboard</a>
    <p style='margin-top:20px;color:#999;font-size:12px'>If you did not expect this request, please log in and decline, then contact the Cooperative Secretary.</p>";
    return emailWrapper('Guarantor Request – Your Response Required', $body);
}
